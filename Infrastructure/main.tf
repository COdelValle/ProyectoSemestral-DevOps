...
resource "aws_instance" "db" {
  ...
  # Usamos la variable protegida inyectada en el arranque de la EC2
  user_data = <<-EOF
    #!/bin/bash
    yum update -y
...
resource "aws_ecs_task_definition" "app" {
  ...
  container_definitions = jsonencode([
    ...
    {
      name  = "ventas-backend"
      image = "${aws_ecr_repository.ventas-backend.repository_url}:latest"
    ...
    # 2. DESPACHO BACKEND
    {
      ...
      image = "${aws_ecr_repository.despacho-backend.repository_url}:latest"
    ...
    # 3. FRONTEND
    {
      ...
      dependsOn = [
        ...
        { containerName = "despacho-backend", condition = "START" }
      ]
      logConfiguration = {
        logDriver = "awslogs"
        options = {
